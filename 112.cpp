...
//Функция генерирует неповторяющиеся цифры
vector<int> initialDigits();

//Функция ввода пользователем цифр
vector<int> getUserNumber();

//Функция проверяет количество быков и коров
...
{
NewGame:
    //Массив для записи количества быков и коров
    vector<int> bullsAndCows;
    //генерирует цифры
    vector<int> sequence_nums;
    //догадки пользователя
...
vector<int> initialDigits() {
    ...
    vector<int> ret;
    for (int i = 0; i < 4; i++) {
        int position = rand() % seq.size(); //рандомное расположение элемента
